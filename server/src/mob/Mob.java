        ...
        public List<ItemMap> getItemMobReward(Player player, int x, int yEnd) {
            ...
            if (MapService.gI().isMapNguHanhSon(mapid)) {
                if (Util.isTrue(7, 100)) {
                    ItemMap it = new ItemMap(zone, 1997, 1, x, yEnd, player.id);
                    it.options.add(new Item.ItemOption(31, 1));
                    list.add(it);
            }
            }
            if (MapService.gI().isMapDoanhTrai(mapid)
                ...
                it.options.add(new Item.ItemOption(100, 1));
                list.add(it);
            }
          if (MapService.gI().isMap3Planets(mapid) && Util.isTrue(20, 100)) {
    int ngoc = Util.nextInt(10, 51); // Random từ 30 đến 70 (có cả 70)
    player.inventory.gem += ngoc; // ✅ đây là ngọc xanh

    Service.gI().sendMoney(player); // ✅ bắt buộc để client hiển thị
    Service.gI().sendThongBao(player, "Bạn nhận được " + ngoc + " ngọc!");
}
            if (MapService.gI().isMap3Planets(mapid)
            ...
            if (player.isActive()
        && player.setClothes.checkSetDes()
        && MapService.gI().isMapNgucTu(mapid)
        && player.nPoint.power >= 60_000_000_000L) { // 👈 điều kiện >= 50 tỷ power
    if (Util.isTrue(10, 100)) { // tỷ lệ rơi 10%
        list.add(new ItemMap(zone, Util.nextInt(1066, 1070), 1, x, yEnd, player.id)); // item 1066–1069
        
    }
}
            if (player.isActive()
        && MapService.gI().isMapHanhTinhThucVat(mapid)
        && player.nPoint.power >= 60_000_000_000L) { // 👈 điều kiện >= 60 tỷ power
    if (Util.isTrue(10, 100)) { // tỷ lệ rơi 10%
         // bi kip tuyet ki
        list.add(new ItemMap(zone, 1229, 1, x, yEnd, player.id)); // item ID 1229
    }
}if (MapService.gI().isMap3Planets(mapid)&&player.setClothes.checkSetEvent()
                    ) {
                if (Util.isTrue(15, 100)
                        ) {
                    int rand = Util.nextInt(0, 4);
                    ItemMap it = new ItemMap(zone, 694 + rand, 1, x, yEnd, player.id);
                    it.options.add(new Item.ItemOption(30, 1));
                    list.add(it);
                }
            }
            if ((Util.isTrue(5, 100) && MapService.gI().isMapNguHanhSon(mapid))) {
                list.add(new ItemMap(zone, Util.nextInt(220,225), 1, x, yEnd, player.id));
            }

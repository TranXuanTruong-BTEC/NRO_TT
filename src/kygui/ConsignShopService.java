...
public class ConsignShopService {
    ...
    private List<ConsignItem> getConsignItem2(Player pl, byte tab, byte to, byte max) {
    ...
    private List<ConsignItem> getConsignItem(Player pl, byte tab, byte... max) {
        List<ConsignItem> its = new ArrayList<>();
        List<ConsignItem> listSort = new ArrayList<>();
        List<ConsignItem> listSort2 = new ArrayList<>();
    ...
    private List<ConsignItem> getConsignItem() {
        List<ConsignItem> its = new ArrayList<>();
        List<ConsignItem> listSort = new ArrayList<>();
    ...
    public ConsignItem getItemBuy(int id) {
        for (ConsignItem it : getConsignItem()) {
            if (it != null && it.id == id) {
                return it;
            }
        }
    ...
    public void openShopKyGui(Player pl, byte index, int page) {
        if (page > getConsignItem(pl, index).size()) {
            return;
        }
        Message msg = null;
        try {
            ...
            List<ConsignItem> items = getConsignItem(pl, index);
            List<ConsignItem> itemsSend = getConsignItem2(pl, index, (byte) (page * 20), (byte) (page * 20 + 20));
    ...
    public void claimOrDel(Player pl, byte action, int id) {
        ...
        switch (action) {
            ...
            case 2: // nhận tiền
                ...
                if (it.goldSell > 0) {
                    Item tvAdd = ItemService.gI().createNewItem((short) 190);
        ...
        try {
            ...
            for (byte i = 0; i < 5; i++) {
                if (i == 4) {
                    ...
                    List<ConsignItem> items = getConsignItem(pl, i);
                    List<ConsignItem> itemsSend = getConsignItem(pl, i, (byte) 20);
                    msg.writer().writeUTF(ConsignShopManager.gI().tabName[i]);

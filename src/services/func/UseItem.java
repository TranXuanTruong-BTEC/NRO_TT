...
public class UseItem {
                ...
                switch (item.template.type) {
                    ...
                    default:
                        switch (item.template.id) {
                            ...
                            case 1538:
                                UseItem.gI().OpenHopQuaVip(pl, 1538);
                                break;
                            case 1775:
                                UseItem.gI().OpenHopThanlinh(pl, 1775);
                                break;
                            case 736:
    ...
    public void openRuongGo(Player player) {
        ...
        if (ruongGo != null) {
            ...
            if (InventoryService.gI().getCountEmptyBag(player) < requiredSlots) {
                ...
                int[] rewardItems = {17, 18, 19, 20, 457, 380, 1229};
   ...
   public void OpenHopQuaThuong(Player player, int itemUseiD) {
    if (InventoryService.gI().getCountEmptyBag(player) > 0) {
        Item itemused = InventoryService.gI().findItemBagByTemp(player, 1537);

        // Danh sách item có thể nhận
        int[] itemIds = {740, 800, 805, 1000, 1414, 1435, 1458, 1482, 1497, 1150, 1151, 1152, 1153, 1154};
        ...
        // Tạo item
        Item rewardItem = ItemService.gI().createNewItem((short) itemId);

        // Thêm 3 chỉ số (50, 77, 103) mỗi cái từ 10-15
        if(itemId<1150||itemId>1154){
        rewardItem.itemOptions.add(new ItemOption(50, Util.nextInt(10, 16)));
        rewardItem.itemOptions.add(new ItemOption(77, Util.nextInt(10, 16)));
        rewardItem.itemOptions.add(new ItemOption(103, Util.nextInt(10, 16)));
        rewardItem.itemOptions.add(new ItemOption(231,1));
        }
        // Cho vào túi
        ...
        Service.gI().sendThongBao(player, "Bạn vừa nhận được vật phẩm đặc biệt!");
    } else {
        Service.gI().sendThongBao(player, "Yêu cầu ít nhất 1 ô trống hành trang");
    }
}
   public void OpenHopQuaVip(Player player, int itemUseiD) {
    if (InventoryService.gI().getCountEmptyBag(player) > 0) {
        Item itemused = InventoryService.gI().findItemBagByTemp(player, 1538);

        int chance = Util.nextInt(1, 101); // random từ 1 đến 100

        Item rewardItem;

        if (chance <= 10) {
            // ---- LOẠI 1 (10%): item 1567 hoặc 1632 ----
            int[] itemType1 = {1567, 1632};
            int itemId = itemType1[Util.nextInt(0, itemType1.length)];

            rewardItem = ItemService.gI().createNewItem((short) itemId);

            rewardItem.itemOptions.add(new ItemOption(77, Util.nextInt(20, 28)));
            rewardItem.itemOptions.add(new ItemOption(103, Util.nextInt(20, 28)));
            rewardItem.itemOptions.add(new ItemOption(50, Util.nextInt(20, 28)));
            rewardItem.itemOptions.add(new ItemOption(101, Util.nextInt(20, 28)));
            rewardItem.itemOptions.add(new ItemOption(95, Util.nextInt(10, 16)));
            rewardItem.itemOptions.add(new ItemOption(96, Util.nextInt(10, 16)));
            rewardItem.itemOptions.add(new ItemOption(231, 1)); // vĩnh viễn

        } else {
            // ---- LOẠI 2 (90%): item 230 → 281 ----
            int itemId = Util.nextInt(230, 281); // 230 đến 281
            rewardItem = ItemService.gI().createNewItem((short) itemId);

            // Thêm các chỉ số đặc biệt tùy theo khoảng item
            if (itemId >= 230 && itemId <= 241) {
                rewardItem.itemOptions.add(new ItemOption(47, Util.nextInt(450, 501)));
            } else if (itemId >= 242 && itemId <= 253) {
                rewardItem.itemOptions.add(new ItemOption(6, Util.nextInt(24000, 30001)));
                rewardItem.itemOptions.add(new ItemOption(27, Util.nextInt(3600, 5001)));
            } else if (itemId >= 254 && itemId <= 265) {
                rewardItem.itemOptions.add(new ItemOption(0, Util.nextInt(2250, 3001)));
            } else if (itemId >= 266 && itemId <= 277) {
                rewardItem.itemOptions.add(new ItemOption(7, Util.nextInt(23000, 30001)));
                rewardItem.itemOptions.add(new ItemOption(28, Util.nextInt(2800, 4001)));
            } else if (itemId >= 278 && itemId <= 281) {
                rewardItem.itemOptions.add(new ItemOption(14, Util.nextInt(10, 16)));
            }

            // Thêm option lặp theo pattern [(245-248), (237-240), (241-244)] với param = 1
            int[] patternStart = {245, 237, 241};
            int mod = (itemId - 230) % 12; // pattern dài 12
            int patternIndex = mod / 4; // mỗi khối 4 item

            for (int i = 0; i < 4; i++) {
                rewardItem.itemOptions.add(new ItemOption(patternStart[patternIndex] + i, 1));
            }
        }

        // Add vào túi, trừ vật phẩm mở, thông báo
        InventoryService.gI().addItemBag(player, rewardItem);
        InventoryService.gI().subQuantityItemsBag(player, itemused, 1);
        InventoryService.gI().sendItemBags(player);
